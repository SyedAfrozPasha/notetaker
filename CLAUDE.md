...
- `recorder.py` — background process (spawned by `start`, killed via SIGTERM by `stop`, SIGKILL after a 30s grace period). `LiveCapture` keeps two `sounddevice` input streams open for the whole session — your microphone (macOS default input; disabled by `capture_microphone: false`) and BlackHole (meeting audio) — and writes one stereo WAV per chunk, **left = Me, right = Others**. Capture stays on the main thread; a single worker thread transcribes finished chunks in order and deletes them, so time spent in Whisper never becomes unrecorded audio. A failed transcription of one chunk is logged into the transcript and recording continues; only a capture failure ends the session. If the Others channel stays silent for 3 chunks while Me has audio, a routing warning line is appended to the transcript once (macOS output switched away from the Multi-Output Device). `check_output_routing`/`check_microphone_routing` give `start` its up-front warnings (returned on `SessionInfo.warnings`, shown by every UI surface).
- `transcriber.py` — wraps `faster-whisper`; transcribes each chunk and appends timestamped lines to a running transcript file. A stereo chunk is transcribed per channel (as float32 arrays, one Whisper pass each) and merged by segment start time into `[hh:mm:ss] Me: ...` / `[hh:mm:ss] Others: ...` lines; a mono chunk gives the plain `[hh:mm:ss] text` line. `model_path` (config `whisper_model_path`) loads a local CTranslate2 model directory with `local_files_only=True` for machines that can't reach Hugging Face.
- `summarizer.py` — defines the `Provider` interface (`summarize(transcript) -> Summary`) with `ClaudeProvider` and `AppleLocalProvider` (apfel, the default) implementations; this is the seam for adding other AI backends later. Each provider carries a `chunk_token_limit` (1800 for Apple's 4K-context on-device model, 12000 for Claude); `summarize_transcript` strips `[hh:mm:ss]` timestamps, map-reduces over chunks at that limit, and reports `on_progress(done, total)` so `stop` can show "Summarizing... chunk 3 of 7". The prompt asks for meeting minutes (discussion, decisions, open questions) and `Owner: task` action items using the `Me:`/`Others:` labels. JSON parsing tolerates prose around the object, which small models add.
...
- `config.py` — loads/validates `~/.notetaker/config.yaml`. Defaults are fully on-device (`ai_provider: apple_local`, `capture_microphone: true`); optional `whisper_model_path` (offline model dir). `update_config` resets `ai_model` to the new provider's default when the provider changes without an explicit model.
